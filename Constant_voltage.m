...
% The input to the function is the charging time tf, initial state of the system x0 and input i. 
% For this function, the model input is the voltage, not the current. The model outputs are
% the voltages (y_store), currents (i_store), simulation times (t_store)
% and model states (states). The model parameters were obtained from the super_params.m file.
...
function [y_store,i_store,t_store,states] = Constant_voltage(tf,x0,i)
...
Current_Ones = ones(Na+Nb+Nc-3,1);
...
%% BC's Phi1
D_volt1_a = [(2/La)*D_cheba(1,Na+1), 0,1/sigma,0;
    (2/La)*D_cheba(Na+1,Na+1),0,0,0;
    0,(2/Lc)*D_chebc(1,1),0,0;
    0, (2/Lc)*D_chebc(Nc+1,1),0,1/sigma];
...
D_volt1_c = [-(2/La)*D_cheba(1,1), 0;
    -(2/La)*D_cheba(Na+1,1), 0;
    0,-(2/Lc)*D_chebc(1,Nc+1);
    0, -(2/Lc)*D_chebc(Nc+1,Nc+1)];

D_volt1_int = D_volt1_a\D_volt1_b;
D_volt1_ext = D_volt1_a\D_volt1_c;
...
B_1 = zeros(Na-1+Nb-1+Nc-1,2);

%% EQUATION 2 - Double layer equatio
DD_ex_eq2 = zeros(Na-1+Nb-1+Nc-1,2);
DD_ex_eq2_B = zeros(Na-1+Nb-1+Nc-1,2);

DD_ex_eq2(1:Na-1,1) = (4/La^2)*DD_cheba(2:Na,Na+1);
DD_ex_eq2(Na+Nb-1:Na+Nb+Nc-3,2) = (4/Lc^2)*DD_chebc(2:Nc,1);

DD_ex_eq2_B(1:Na-1,1) = (4/La^2)*DD_cheba(2:Na,1);
DD_ex_eq2_B(Na+Nb-1:Na+Nb+Nc-3,2) = (4/Lc^2)*DD_chebc(2:Nc,Nc+1);
...
A_in_2 = [zeros(Na-1+Nb-1+Nc-1,Na-1+Nb-1+Nc-1),sigma*DD_in_eq2,zeros(Na-1+Nb-1+Nc-1,Na-1+Nb-1+Nc-1)];
B_in_2 = sigma*DD_ex_eq2_B;

A_ex_2 = [zeros(Na-1+Nb-1+Nc-1,Na-1+Nb-1+Nc-1),sigma*DD_ex_eq2*(D_volt1_int(1:2,:)),zeros(Na-1+Nb-1+Nc-1,Na-1+Nb-1+Nc-1)];
B_ex_2 = sigma*DD_ex_eq2*(D_volt1_ext(1:2,:));

A_2 = A_in_2+A_ex_2;
B_2 = B_in_2+B_ex_2;

%% EQUATION 3 - Algebraic Constraint for current conservation
D_ex_eq3_volt1 = zeros(Na-1+Nb-1+Nc-1,2);
D_ex_eq3_volt1_B = zeros(Na-1+Nb-1+Nc-1,2);

D_ex_eq3_volt1(1:Na-1,1) = (2/La)*D_cheba(2:Na,Na+1);
D_ex_eq3_volt1(Na+Nb-1:Na+Nb+Nc-3,2) = (2/Lc)*D_chebc(2:Nc,1);

D_ex_eq3_volt1_B(1:Na-1,1) = (2/La)*D_cheba(2:Na,1);
D_ex_eq3_volt1_B(Na+Nb-1:Na+Nb+Nc-3,2) = (2/Lc)*D_chebc(2:Nc,Nc+1);

D_ex_eq3_volt1 = sigma*D_ex_eq3_volt1;
D_ex_eq3_volt1_B = sigma*D_ex_eq3_volt1_B;
...
B_in_3 = D_ex_eq3_volt1_B;

A_ex_3 = [zeros(Na-1+Nb-1+Nc-1,Na-1+Nb-1+Nc-1),D_ex_eq3_volt1*D_volt1_int(1:2,:)+Current_Ones*D_volt1_int(number,:),D_ex_eq3_volt2*(D_volt2_a\D_volt2_c)];
A_ex_lnc = D_ex_eq3_volt2*(D_volt2_a\D_volt2_d);
A_ex_lnc_ex_BC = D_ex_eq3_volt2*(D_volt2_a\(-D_volt2_b));
A_ex_lnc_ex = D_ex_eq3_lnc;
B_ex_3 = D_ex_eq3_volt1*D_volt1_ext(1:2,:)+Current_Ones*D_volt1_ext(number,:);%% Is this right?????
...
%% Simulation
rhsfun = @(t,X) (A*X+A_lnc*log(X(1:Na-1+Nb-1+Nc-1,1))+A_lnextc*log((Dc_a\Dc_b)*X(1:Na-1+Nb-1+Nc-1,1))+B*i); 
options = odeset('Mass',M);
[t,states] = ode15s(rhsfun,[0 tf],x0,options); 
size_states = (size(states,1));

%% OUTPUT MATRICES
C_int = D_volt1_int(number,:);
D = D_volt1_ext(number,:);
...
for j = 1:size_states
    ...
    i_store1(j) = i(1);
    i_store2(j) = i(2);
    y_store(j) = i_store1(j)-i_store2(j);
    t_store(j) = t(j);  
end
